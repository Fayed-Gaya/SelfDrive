...
def look_ahead_pt_calculator(current_pose_orientation, current_pos, l_d):
    ...
    look_ahead_y = current_pos[1] + l_d * np.sin(yaw)

    look_ahead_pt = np.array([look_ahead_x, look_ahead_y])

    return look_ahead_pt

def nearest_waypoint_lookahead_point(reference_path, lookahead_point):
    """
    Calculate the nearest waypoint to the look ahead point along the reference path.
    """

    # Calculate an array of all of the distances between the current pose and the poses of the waypoints that make up the reference path
    distance_array = np.linalg.norm(lookahead_point - reference_path, axis=1)
...
def calculate_heading_error(current_pos, nn1, look_ahead_point, nn2):
    # Calculate vector 1
    v1 = (look_ahead_point - current_pos) / (np.linalg.norm(current_pos - look_ahead_point) + 1e-6)
    # Calcuate vector 2
    v2 = (nn2 - nn1) / (np.linalg.norm(nn1 - nn2) + 1e-6)
    # Calculate heading error
    heading_error = np.arctan2(np.dot(np.cross(v1, v2), np.array([0,0,1])),  np.dot(v1, v2))
...
def current_pose_callback(data, args):
    ...
    # Calculate the nearest waypoint to the the lookahead point
    lh_np = nearest_waypoint_lookahead_point(reference_path, look_ahead_pt)
    ...
    # Calculate the heading error
    heading_error = calculate_heading_error(current_pose, np_actual, look_ahead_pt, lh_np)
    ...
    # print('Pure Pursuit %s' % pursit_steering_angle)
    ...
    drive = AckermannDrive(steering_angle=stanely_steering_angle, speed=VEL)
...
if __name__ == "__main__":
    ...
    pub_init_pose = rospy.Publisher(init_pose_topic, PoseWithCovarianceStamped, queue_size=5)
